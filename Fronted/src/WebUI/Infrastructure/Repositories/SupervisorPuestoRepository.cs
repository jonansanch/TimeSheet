...
public class SupervisorPuestoRepository(HttpClient http, AuthStateService authState)
    : ISupervisorPuestoRepository
{
    public async Task<List<SupervisorPuestoResponse>> GetAllAsync(CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(authState.AccessToken)) return [];

        using var message = CreateMessage(HttpMethod.Get, "api/supervisores-puesto");
        var response = await http.SendAsync(message, ct);
        if (!response.IsSuccessStatusCode) return [];

        return await response.Content.ReadFromJsonAsync<List<SupervisorPuestoResponse>>(cancellationToken: ct) ?? [];
    }

    public Task<(bool Ok, SupervisorPuestoResponse? Item, string? Error)> CrearAsync(
        GuardarSupervisorPuestoRequest request, CancellationToken ct = default) =>
        EnviarAsync(HttpMethod.Post, "api/supervisores-puesto", request, ct);

    public Task<(bool Ok, SupervisorPuestoResponse? Item, string? Error)> ActualizarAsync(
        int id, GuardarSupervisorPuestoRequest request, CancellationToken ct = default) =>
        EnviarAsync(HttpMethod.Put, $"api/supervisores-puesto/{id}", request, ct);

    public async Task<(bool Ok, SupervisorPuestoResponse? Item)> ToggleAsync(int id, CancellationToken ct = default)
    {
        var (ok, item, _) = await EnviarAsync<object?>(
            HttpMethod.Put, $"api/supervisores-puesto/{id}/toggle", null, ct);
        return (ok, item);
    }

    private async Task<(bool Ok, SupervisorPuestoResponse? Item, string? Error)> EnviarAsync<TBody>(
        HttpMethod method, string url, TBody body, CancellationToken ct)
    {
        ...
        var item = await response.Content.ReadFromJsonAsync<SupervisorPuestoResponse>(cancellationToken: ct);
